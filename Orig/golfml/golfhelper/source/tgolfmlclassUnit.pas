...
uses
  Classes, SysUtils, FileUtil,XMLRead, XMLWrite, DOM;
...
// Add more Error Codes and Error Strings as needed in Get/Set routines
  C_ERROR_NOERROR = 0;
  C_ERROR_BADPATH = 1;
  C_ERRORSTRING_BADPATH = 'Bad CourseXMLPath';
...
type
  ...
  private
    ...
    procedure priv_ProcessXMLTee(aNode: TDomNode; TeeColour: cardinal;
      sCourseName: string);
  ...
  public
    ...
    function IsValidGolfmlFile:Boolean;  // CourseXMLPath must be set first

    // The main methods
    function MakeGolfmlFile: boolean;
    function GetCourseInfoFromFile: boolean; // CourseXMLPath must be set first
  ...
  published
    ...
    property CourseNumberOfHoles: cardinal
      read priv_GetCourseNumberOfHoles write priv_SetCourseNumberOfHoles;
    ...
    // fMetric determines metres or yards
    property CourseTotalPar: cardinal read fCourseTotalPar; // Set when courseindex is set
  ...
  try
    try
      ...
      begin
        ...
        begin
          // <Tee-Set>s
          if Length(fCourseTeeColourArray[cCourseLoop,
            cTeePositionLoop]) = 0 then
            continue;
          ...
          TextNode :=
            Doc.CreateTextNode(
            Format('%d', [fTeeColourSlopeRatingArray[cCourseLoop, cTeePositionLoop]]));
          InnerInnerNode.AppendChild(TextNode);
          InnerNode.AppendChild(InnerInnerNode);
          OuterNode.AppendChild(InnerNode);
          ...
          begin
            ...
            TextNode :=
              Doc.CreateTextNode(Format('%d',
              [fCourseStrokeIndexArray[cCourseLoop, cHoleLoop]]));
            InnerNode.AppendChild(TextNode);
            OuterNode.AppendChild(InnerNode);
            // Distance Metres
...
// *********************************************************************
function TGolfmlClass.IsValidGolfmlFile:Boolean;
var
  Doc: TXMLDocument;
...
begin
  try
    ...
    If CurrentNode.NodeName='country-club' then Result:=True;
  finally
    Doc.Free;
  end;
...
begin
  ...
  if (fMetric = C_YARDS) and (fTotalTeeDistanceYards = 0) and
    (fTotalTeeDistanceMetres > 0) then
  begin
      ...
      begin
        ...
        begin
          raise Exception.Create(C_ERRORXMLREAD +
            'Error: Tee number attribute is invalid');
          Exit;
        end;
      end;
    end;
            ...
            begin
              ...
              begin
                fTeeDistanceArrayYards[fCourseIndex,
                  TeeColour, iTeeNumber - 1] := 0;
                raise Exception.Create(C_ERRORXMLREAD +
                  'Error: A Distance element (yards) is invalid');
              end;
            end;
          end;
        end;
  ...
  begin
    ...
    begin
      if Length(CurrentNode.TextContent) > 0 then
        ...
        sCourseName := 'Unknown Course Name';
...
begin
  ...
  begin
    raise Exception.Create(Format('HoleIndex %d out of bounds (Max:%d)',
      [aCardinal, fCourseNumberOfHoles]));
    Exit;
  end;
  // Assign
